	...
	void view() {
		...
		while(true) {
			int choice= JOptionPane.showOptionDialog(null, "대중교통을 선택하세요!", "TMS",
					...
					null, arName, null);
			if(choice ==-1) break;
			int btnIndex = JOptionPane.showOptionDialog(null, "도착지를 선택해주세요", 
					"TMS",JOptionPane.DEFAULT_OPTION, JOptionPane.PLAIN_MESSAGE,
					null, bus360.arStation, null);
			if(btnIndex ==0&&choice ==0) {
				JOptionPane.showMessageDialog(null, "도착지는 교대 다음역부터 가능합니다.");
				continue;
			}
			switch(choice) {
			//버스
